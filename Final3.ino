...
void setup() {
  ...
  target_count2 = pulses_per_cell2;
...
void loop() {
  ...
  if (distance < 17){   //front wall within 20 cm
...
// PID parameters
float Kp = 6;  // Proportional gain 6 0 0 
...
void move_backward() {
  ...
  while ((encoder1_count < target_count2) && (encoder2_count < target_count2)) {
    mpu.update();
    float currentAngle = mpu.getAngleZ();
    Serial.print("Target "); 
    Serial.println(targetAngle);
